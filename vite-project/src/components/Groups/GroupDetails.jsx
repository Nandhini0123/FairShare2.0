...
export default function GroupDetails({ groupId, onCancel }) {
  ...
  if (!groupData) {
    return <p className="text-center text-gray-500">Loading group details...</p>;
  }
    ...
    <>
      <div className="fixed inset-0 flex items-center justify-center bg-beige 100vh width-full z-50">
        <div className="bg-white w-[55%] h-[90%] p-6 rounded-lg shadow-lg relative overflow-auto">
            <h3 className="text-title text-charcoal mb-4 font-bold">Group Details</h3>
            <button
              onClick={onCancel}
              ...
              {groupData.expenses && groupData.expenses.length > 0 ? (
                <div className="grid grid-cols-1 gap-4">
                  {groupData.expenses.map((expense, index) => (
                    ...
                    >
                      <div>
                        <p className="text-lg text-charcoal font-bold">{expense.expenseName}</p>
                ...
                <p className="text-gray-500">No expenses have been added to this group yet.</p>
              )}
